...
- **端到端测试**：`scripts/test-marketing-e2e.js` 会先确保有会员、商品、促销、优惠券（无则自动创建），再调用价格计算接口（商品 + 会员 + 优惠券 + 促销），并比对预期：原价 > 0、实付 ≥ 0、实付 ≤ 原价；有优惠时预期有节省。运行：`node scripts/test-marketing-e2e.js`（环境变量同回归脚本，可选 `MEMBER_ID`）。
- **计算场景测试**：`scripts/test-marketing-calc-scenarios.js` 会**按脚本内写死的数值**创建一条商品（单价 100）、一条促销（满 100 减 10）、一张优惠券（减 10 元、满 50 可用），然后在四种条件下分别算价并比对预期：① 无促销无券 → 100；② 仅促销 → 90；③ 仅券 → 90；④ 促销+券 → 80。运行：`node scripts/test-marketing-calc-scenarios.js`（环境变量同上）。脚本内常量可改（商品单价、数量、促销/券规则及预期值）。
  - **避免重复创建**：每次运行都会新建商品/促销/券。若希望复用已有数据，可设置 `USE_EXISTING=1` 并同时设置 `PRODUCT_ID`、`SKU_ID`、`PROMOTION_ID`、`COUPON_ID`（首次运行后从日志拿到 ID），后续运行将不再创建、只做算价断言。
  - **清理测试数据**：运行 `node scripts/cleanup-marketing-test-data.js` 可删除名称包含「场景测试」的商品、促销、优惠券（需同环境变量 `BASE_URL`、管理员账号）。

### 后台为何看不到促销 / 优惠券未“发放”给会员

- **促销管理列表**：后台「促销管理」的**类型筛选**原先只有限时抢购、团购、捆绑、包邮；**满减 / 满送 / 满折** 已补全到筛选与表单中，列表默认「全部类型」会显示所有记录。若仍看不到，请确认：① 与脚本/接口使用同一 `BASE_URL`（同一环境、同一库）；② 列表分页调到第 1 页、适当增大每页条数。
- **优惠券与会员**：当前设计下，优惠券创建后为「可领取」池；**算价接口不要求会员先领取**，传 `appliedCoupons: [couponId]` 即可参与抵扣。若希望会员在小程序「我的优惠券」里看到并领取，需小程序端调用 `POST /api/miniapp/coupons/:id/receive`；脚本仅测算价逻辑，不会调用领取接口。
